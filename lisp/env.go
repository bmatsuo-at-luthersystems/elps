...
//
// BUG: When invoked by builtin functions ``load-string'', ``load-bytes'', etc
// the stack observed by the loaded code will not reflect the caller's stack.
// Furthermore, any error generated by evaluating LVals parsed from the source
// stream will not reflect the stack at the callsite of ``load-string'',
// ``load-bytes'', etc.
